...
#define I2C_START_TIMEOUT 100
#define I2C_ADDR_TIMEOUT  100
#define I2C_TX_TIMEOUT    100
#define I2C_RX_TIMEOUT    100

#define I2C_SEND_START(i2c) do{	  \
		i2c_send_start(i2c); \
		SAFE_SPIN_LOCK(!((I2C_SR1(i2c) & I2C_SR1_SB) \
...
#define I2C_SEND_ADDRESS(i2c, addr, write) do{	  \
		i2c_send_7bit_address(i2c, addr, write); \
		SAFE_SPIN_LOCK(!(I2C_SR1(i2c) & I2C_SR1_ADDR), \
		               I2C_ADDR_TIMEOUT, \
		               EC_I2C_ADDRESS_NACK, \
		               STATUS_I2C_ADDRESS_NACK); \
	} while (0)

#define I2C_SEND_DATA(i2c, data) do{	  \
		i2c_send_data(i2c, data); \
		SAFE_SPIN_LOCK(!(I2C_SR1(i2c) & (I2C_SR1_BTF | I2C_SR1_TxE)), \
		               I2C_TX_TIMEOUT, \
		               EC_I2C_TX_TIMEOUT, \
		               STATUS_I2C_TX_TIMEOUT); \
	} while (0)

#define I2C_RECEIVE_DATA(i2c) do{	  \
		SAFE_SPIN_LOCK(!(I2C_SR1(i2c) & I2C_SR1_RxNE), \
		               I2C_RX_TIMEOUT, \
		               EC_I2C_RX_TIMEOUT, \
		               STATUS_I2C_RX_TIMEOUT); \
	} while (0)
...
uint32_t i2c_write_byte_raw(uint32_t i2c, uint8_t device_address, uint8_t data) {
	uint32_t reg32 __attribute__((unused));

	I2C_SEND_START(i2c);

	I2C_SEND_ADDRESS(i2c, device_address, I2C_WRITE);
...
uint16_t i2c_read_word_raw(uint32_t i2c, uint8_t device_address) {
	...
	I2C_SEND_ADDRESS(i2c, device_address, I2C_READ);

	reg32 = I2C_SR2(i2c);

	I2C_CR1(i2c) |= I2C_CR1_ACK;
	I2C_RECEIVE_DATA(i2c);
