...
#define card_enable()               gpio_clear(GPIOA, GPIO15);
#define card_disable()              gpio_set(GPIOA, GPIO15);
...
uint8_t sdcard2_setup(void) {
	...
	buff[0] = spi2_write_and_read(DUMMY_BYTE);
	buff[1] = spi2_write_and_read(DUMMY_BYTE);
	buff[2] = spi2_write_and_read(DUMMY_BYTE);
	buff[3] = spi2_write_and_read(DUMMY_BYTE);
	card_disable();
	spi2_write_and_read(DUMMY_BYTE);
...
void sdcard2_single_block_read(uint32_t address, uint8_t *block) {
...
//		printf("Read Error1: %d\r\n", (int)r1);
	...
	for (i = 0; i < 4000; ++i) {
		r1 = spi2_write_and_read(DUMMY_BYTE);
		if (r1 == 0xFE) {
			break;
		}
	}
...
//		printf("Read Error2: %d\r\n", (int)r1);
	...
	for (i = 0; i < 512; ++i) {
		block[i] = spi2_write_and_read(DUMMY_BYTE);
	}

	// Read CRC
	uint16_t crc = 0;
	crc = spi2_write_and_read(DUMMY_BYTE);
	crc <<= 8;
	crc |= spi2_write_and_read(DUMMY_BYTE);
...
void sdcard2_single_block_write(uint32_t address, uint8_t *block) {
...
//		printf("Write Error1: %d\r\n", (int)r1);
	...
	spi2_write_and_read(0xFE);
	for (i = 0; i < 512; ++i) {
		spi2_write_and_read(block[i]);
	}
	...
	i = 0;
	while ((r1 = spi2_write_and_read(DUMMY_BYTE)) == 0xFF && i < 10000) ++i;

	if (i == 10000) {
		card_disable();
		spi2_write_and_read(0xFF);
		return;
	}
...
//		printf("Write error2: %x\r\n", r1);
	...
	i = 0;
	while (spi2_write_and_read(DUMMY_BYTE) != 0xFF) ++i;

	card_disable();
	spi2_write_and_read(DUMMY_BYTE);

	r1 = send_command_without_disabling(13, 0, 0);
	uint8_t r2 = spi2_write_and_read(DUMMY_BYTE);
...
void sdcard2_multiple_block_write(uint32_t address, uint8_t *block) {
	if (write_started) {
		uint8_t r1;
		uint32_t i;
		// Start data packet transmission
		...
		// We should wait for a Data Response byte
		i = 0;
		while ((r1 = spi2_write_and_read(DUMMY_BYTE)) == 0xFF && i < 100000);
		...
		// During the writing operation sd card holds data line low
		i = 0;
		while (spi2_write_and_read(DUMMY_BYTE) != 0xFF && i < 1000) ++i;
...
uint8_t send_command_without_disabling(uint8_t cmd, uint32_t arg, uint8_t crc) {
	...
	spi2_write_and_read(cmd | 0x40);
	spi2_write_and_read(arg >> 24);
	spi2_write_and_read(arg >> 16);
	spi2_write_and_read(arg >> 8);
	spi2_write_and_read(arg);
	spi2_write_and_read(crc);
